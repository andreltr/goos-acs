...
SBIG_FILE_ERROR CSBIGImg::SaveFITS(const char *filename)
{
    ...
    fits_write_key(fptr, TSTRING, FITS_KEYS[FK_OBJ],	(void *)m_cFITSObject.c_str(), "", &status);
    fits_write_key(fptr, TSTRING, FITS_KEYS[FK_SCOPE],	(void *)m_cFITSTelescope.c_str(), "", &status);
    fits_write_key(fptr, TSTRING, FITS_KEYS[FK_INST],	(void *)m_cCameraModel.c_str(), "Camera Model", &status);
    fits_write_key(fptr, TSTRING, FITS_KEYS[FK_OBS],	(void *)m_cObserver.c_str(), "", &status);
	fits_write_key(fptr, TSTRING, FITS_KEYS[FK_DATEOBS],(void *)timeBuf, "GMT START OF EXPOSURE", &status);
    fits_write_key(fptr, TDOUBLE, FITS_KEYS[FK_EXP],	(void *)&m_dExposureTime, "EXPOSURE IN SECONDS", &status);
    fits_write_key(fptr, TDOUBLE, FITS_KEYS[FK_CCDT],	(void *)&m_dCCDTemperature, "CCD TEMP IN DEGREES C", &status);
    fits_write_key(fptr, TDOUBLE, FITS_KEYS[FK_PIXX],	(void *)&pixWidth, "PIXEL WIDTH IN MICRONS", &status);
    fits_write_key(fptr, TDOUBLE, FITS_KEYS[FK_PIXY],	(void *)&pixHeight, "PIXEL HEIGHT IN MICRONS", &status);
    fits_write_key(fptr, TUSHORT, FITS_KEYS[FK_BINX],	(void *)&m_uHorizontalBinning, "HORIZONTAL BINNING FACTOR", &status);
    fits_write_key(fptr, TUSHORT, FITS_KEYS[FK_BINY],	(void *)&m_uVerticalBinning, "VERTICAL BINNING FACTOR", &status);
    fits_write_key(fptr, TINT,    FITS_KEYS[FK_ORGX],	(void *)&m_nSubFrameLeft, "SUB_FRAME ORIGIN X_POS", &status);
    fits_write_key(fptr, TINT,    FITS_KEYS[FK_ORGY],	(void *)&m_nSubFrameTop, "SUB_FRAME ORIGIN Y_POS", &status);
    fits_write_key(fptr, TDOUBLE, FITS_KEYS[FK_EGAIN],	(void *)&m_dEGain, "ELECTRONS PER ADU", &status);
    fits_write_key(fptr, TDOUBLE, FITS_KEYS[FK_FL],		(void *)&focalLen, "FOCAL LENGTH IN MM", &status);
    fits_write_key(fptr, TDOUBLE, FITS_KEYS[FK_APTD],	(void *)&apDiam, "APERTURE DIAMETER IN MM", &status);
    fits_write_key(fptr, TDOUBLE, FITS_KEYS[FK_APTA],	(void *)&apArea, "APERTURE AREA IN SQ-MM", &status);
    fits_write_key(fptr, TLONG,	  FITS_KEYS[FK_BLACK],	(void *)&m_lBackground, "BLACK ADU FOR DISPLAY", &status);
    fits_write_key(fptr, TLONG,   FITS_KEYS[FK_WHITE],	(void *)&fitsWhite, "WHITE ADU FOR DISPLAY", &status);
    fits_write_key(fptr, TLONG,   FITS_KEYS[FK_PED],	(void *)&fitsPedestal, "ADD TO ADU FOR 0-BASE", &status);
    fits_write_key(fptr, TUSHORT, FITS_KEYS[FK_MAX],	(void *)&m_uSaturationLevel, "SATURATION LEVEL", &status);
    fits_write_key(fptr, TSTRING, FITS_KEYS[FK_VER],	(void *)"SBFITSEXT Version 1.0", "SBIG FITS EXTENSIONS VER", &status);
    fits_write_key(fptr, TSTRING, FITS_KEYS[FK_SWA],	(void *)m_cSoftware.c_str(), "DATA ACQ SOFTWARE", &status);
    fits_write_key(fptr, TSTRING, FITS_KEYS[FK_SWC],	(void *)m_cSoftware.c_str(), "", &status);
	fits_write_key(fptr, TSTRING, FITS_KEYS[FK_FILT],	(void *)m_cFilter.c_str(), "OPTICAL FILTER NAME", &status);
    fits_write_key(fptr, TUSHORT, FITS_KEYS[FK_SNAP],	(void *)&m_uNumberExposures, "NUMBER IMAGES COADDED", &status);
	fits_write_key(fptr, TSTRING, FITS_KEYS[FK_DATE],	(void *)dateBuf, "GMT DATE WHEN THIS FILE CREATED", &status);
    fits_write_key(fptr, TUSHORT, FITS_KEYS[FK_RES],	(void *)&m_uReadoutMode, "RESOLUTION MODE", &status);
	fits_write_key(fptr, TSTRING, FITS_KEYS[FK_EXPST],	(void *)expStateBuf, "EXPOSURE STATE (HEX)", &status);
    fits_write_key(fptr, TDOUBLE, FITS_KEYS[FK_RESP],	(void *)&m_dResponseFactor, "CCD RESPONSE FACTOR", &status);
	fits_write_key(fptr, TSTRING, FITS_KEYS[FK_NOTE],	(void *)m_cImageNote.c_str(), "", &status);
    fits_write_key(fptr, TDOUBLE, FITS_KEYS[FK_TRAK],	(void *)&m_dTrackExposure, "TRACKING EXPOSURE", &status);
