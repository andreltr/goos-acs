/*
 ...
 1. This software (c)2004-2008 Santa Barbara Instrument Group.
...
#define VERSION_STR		"1.21"	/* version of this class */
 ...
 */
double CSBIGCam::hex2double(unsigned long ul)
{
  double res, mult;
  int i;
...
//  ST1K_CAMERA, ST2K_CAMERA, STL_CAMERA, ST402_CAMERA,
//  STX_CAMERA, ST4K_CAMERA, NEXT_CAMERA, NO_CAMERA=0xFFFF } CAMERA_TYPE;
static const char *CAM_NAMES[] =
{ "Type 0", "Type 1", "Type 2", "Type 3", "ST-7", "ST-8", "ST-5C", "TCE", "ST-237", "ST-K", "ST-9",
    "STV", "ST-10", "ST-1K", "ST-2K", "ST-L", "ST-402", "STX", "ST-4K" };
string CSBIGCam::GetCameraTypeString(void)
{
  ...
  if (m_eCameraType < (CAMERA_TYPE) (sizeof(CAM_NAMES) / sizeof(const char *)))
  {
    ...
    // include the STX sub-models
    if (m_eCameraType == STX_CAMERA)
    {
      gcip.request = CCD_INFO_IMAGING;
      if (SBIGUnivDrvCommand(CC_GET_CCD_INFO, &gcip, &gcir) == CE_NO_ERROR)
      {
        // driver reports name as "SBIG STX-XXX..."
        p1 = gcir.name + 5;
        if ((p2 = strchr(p1, ' ')) != NULL)
        {
          *p2 = 0;
...
PAR_ERROR CSBIGCam::SBIGUnivDrvCommand(short command, void *Params, void *Results)
{
  ...
  else
  {
    ...
    if ((m_eLastError = (PAR_ERROR) ::SBIGUnivDrvCommand(CC_SET_DRIVER_HANDLE, &sdhp, NULL))
        == CE_NO_ERROR)
...
PAR_ERROR CSBIGCam::GrabSetup(CSBIGImg *pImg, SBIG_DARK_FRAME dark)
{
  ...
  pImg->SetPixelHeight(hex2double(gcir.readoutInfo[m_sGrabInfo.rm].pixelHeight)
      * m_sGrabInfo.vertNBinning / 1000.0);
  ...
  if (m_eCameraType == ST5C_CAMERA || (m_eCameraType == ST237_CAMERA && s.find("ST-237A", 0)
      == string::npos))
...
PAR_ERROR CSBIGCam::GrabMain(CSBIGImg *pImg, SBIG_DARK_FRAME dark)
{
  ...
  if (pImg->GetImageNote().length() == 0)
  {
    ...
    sprintf(cs, "Local time:%d/%d/%4d at %d:%02d:%02d", pLT->tm_mon + 1, pLT->tm_mday, pLT->tm_year
        + 1900, pLT->tm_hour, pLT->tm_min, pLT->tm_sec);
...
PAR_ERROR CSBIGCam::StartReadout(unsigned short top, unsigned short left, unsigned short height,
...
PAR_ERROR CSBIGCam::FastReadoutLine(ReadoutLineParams* rlp, unsigned short *dest)
{
  if (CheckLink())
  {
    return SBIGUnivDrvCommand(CC_READOUT_LINE, rlp, dest);
 ...
 */
PAR_ERROR CSBIGCam::QueryTemperatureStatus(MY_LOGICAL &enabled, double &ccdTemp, double& airTemp,
    double &setpointTemp, double &percentTE)
{
  ...
  if (CheckLink())
  {
    if (SBIGUnivDrvCommand(CC_QUERY_TEMPERATURE_STATUS, NULL, &qtsr) == CE_NO_ERROR)
    {
      ...
      airTemp = ADToDegreesC(qtsr.ambientThermistor, FALSE);
...
{
  double airTemp, setpointTemp, percentTE;
  MY_LOGICAL teEnabled;

  return QueryTemperatureStatus(teEnabled, ccdTemp, airTemp, setpointTemp, percentTE);
}
...
/*
 ...
 Read and return the current air temperature.

 */
PAR_ERROR CSBIGCam::GetAirTemperature(double &airTemp)
{
...
PAR_ERROR CSBIGCam::SetShutter(SHUTTER_COMMAND command)
{
  return SetMiscellaneousParams(m_fanEnable, command, m_ledState);
}

/*

 SetMiscellaneousParams:

 Set the misc. parameters.

 */

PAR_ERROR CSBIGCam::SetMiscellaneousParams(MY_LOGICAL fanEnable, SHUTTER_COMMAND shutterCommand,
    LED_STATE ledState)
{
  if (CheckLink())
  {
    MiscellaneousControlParams mp;
    mp.fanEnable = fanEnable;
    mp.shutterCommand = shutterCommand;
    mp.ledState = ledState;

    if (SBIGUnivDrvCommand(CC_MISCELLANEOUS_CONTROL, &mp, NULL) == CE_NO_ERROR)
    {
      return GetMiscellaneousParams();
    }
  }
  return m_eLastError;
}
...
/*
 ...
 GetMiscellaneousParams:

 Get the camera's status of fan, shutter and leds.

 */

PAR_ERROR CSBIGCam::GetMiscellaneousParams(void)
{
  if (CheckLink())
  {
    QueryCommandStatusParams qcsp;
    ...
    if (SBIGUnivDrvCommand(CC_QUERY_COMMAND_STATUS, &qcsp, &qcsr) == CE_NO_ERROR)
    {
      m_ledState = static_cast<LED_STATE> ((qcsr.status & 0x1800) >> 11);
      m_shutterState = static_cast<SHUTTER_STATE7> ((qcsr.status & 0x600) >> 9);
      m_fanEnable = static_cast<MY_LOGICAL> ((qcsr.status & 0x100) >> 8);
      m_shutterCommand = static_cast<SHUTTER_COMMAND> (qcsr.status & 0xff);
    }
  }
  return m_eLastError;
}
...
PAR_ERROR CSBIGCam::IsRelayActive(CAMERA_RELAY relay, MY_LOGICAL &active)
{
  ...
  if (CheckLink())
  {
    ...
    if (SBIGUnivDrvCommand(CC_QUERY_COMMAND_STATUS, &qcsp, &qcsr) == CE_NO_ERROR)
    {
      ...
      {
      case RELAY_XPLUS:
        active = ((qcsr.status & 8) != 0);
        break;
      case RELAY_XMINUS:
        active = ((qcsr.status & 4) != 0);
        break;
      case RELAY_YPLUS:
        active = ((qcsr.status & 2) != 0);
        break;
      case RELAY_YMINUS:
        active = ((qcsr.status & 1) != 0);
        break;
      }
    }
  }
...
{ "ST-7", "ST-8", "ST-5C", "TCE", "ST-237", "STK", "ST-9", "STV", "ST-10", "ST-1001", "ST-2000",
    "STL", "ST-402" };
static const char *AD_RES_NAMES[] =
{ "Unknown", "12 Bits", "16 Bits" };
static const char *CFW_NAMES[] =
{ "Unknown", "External", "Shutter Wheel", "CFW-5C" };
PAR_ERROR CSBIGCam::GetFormattedCameraInfo(string &ciStr, MY_LOGICAL htmlFormat /* = TRUE */)
{
  ...
  do
  {
    ...
    if ((res = SBIGUnivDrvCommand(CC_GET_CCD_INFO, &gcip, &gcir0)) != CE_NO_ERROR)
      break;
    if (gcir0.cameraType >= ST7_CAMERA && gcir0.cameraType <= ST402_CAMERA)
    {
    ...
    {
      if (gcir3.adSize > AD_16_BITS)
        gcir3.adSize = AD_UNKNOWN;
      ciStr += ca + "A/D Resolution:" + cs + AD_RES_NAMES[gcir3.adSize] + cb;
      if (gcir3.filterType > FW_FILTER_WHEEL)
        gcir3.filterType = FW_UNKNOWN;
    ...
    {
      sprintf(c, "%s", gcir4.capabilitiesBits & CB_CCD_EXT_TRACKER_MASK ? "Can be used"
          : "Doesn't Support");
    ...
    sprintf(c, "%1.2lf x %1.2lf microns", hex2double(gcir0.readoutInfo[0].pixelWidth), hex2double(
        gcir0.readoutInfo[0].pixelHeight));
    ...
    if ((res = SBIGUnivDrvCommand(CC_GET_CCD_INFO, &gcip, &gcir4)) != CE_NO_ERROR)
      break;
    sprintf(c, "%s", gcir4.capabilitiesBits & CB_CCD_TYPE_MASK ? "Frame Transfer CCD"
        : "Full Frame CCD");
    ciStr += ca + "CCD Type:" + cs + c + cb;
    sprintf(c, "%s", gcir4.capabilitiesBits & CB_CCD_ESHUTTER_MASK ? "Yes, Supports ms Exposures"
        : "No");
